...
export abstract class AbstractApiService {
  ...
  private static _catchHandler<R> (error: any): WorkDone<R> | Promise<WorkDone<R>> {
    let message = (error && error.message ? error.message : 'Unknown Error (2)') as string
  ...
  // post request
  protected async doPost<R> (serviceUri: string, body: any, config?: AxiosRequestConfig): Promise<WorkDone<R>> {
    if (!serviceUri.startsWith('/')) {
      serviceUri = '/' + serviceUri
    }
    const runConfig = _merge({
    ...
    try {
      const resp = await this._axiosInstance.post<WorkDone<R>>(`${this._serviceApiBaseUrl}${serviceUri}`, body, runConfig)
      return AbstractApiService._thenHandler(resp)
    }
    catch (err) {
      return AbstractApiService._catchHandler(err as AxiosError)
  ...
  // put request
  protected async doPut<R> (serviceUri: string, body: any, config?: AxiosRequestConfig): Promise<WorkDone<R>> {
    if (!serviceUri.startsWith('/')) {
      serviceUri = '/' + serviceUri
    }
    const runConfig = _merge({
    ...
    try {
      const resp = await this._axiosInstance.put<WorkDone<R>>(`${this._serviceApiBaseUrl}${serviceUri}`, body, runConfig)
      return AbstractApiService._thenHandler(resp)
    }
    catch (err) {
      return AbstractApiService._catchHandler(err as AxiosError)
